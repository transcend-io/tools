...
| Tools list empty; server shows disconnected / auth error / 401 at session start | Connection or OAuth session    | Re-run Connect (browser re-auth); check optional `GATEWAY_BASE_URL` (self-host) — not a deny |
