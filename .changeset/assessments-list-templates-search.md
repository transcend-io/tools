...
The dashboard also filters templates by source, creator and last editor, and those are
deliberately left out. They cost around 980 characters of the shared `tools/list` budget and
save nothing at the scale templates actually run at: a cold-read probe answered "which came
from OneTrust" and "what has Daniel built" against a live org, and both were single unfiltered
calls over fourteen rows matched in-model. The data barely discriminates either — thirteen of
fourteen templates are `MANUAL`, none are `IMPORT`, and six have no creator recorded at all,
so a `creatorIds` filter could never match them and the probe came close to reporting that
absence as "nobody built these". `source` still rides along on every row, since row fields cost
nothing in the descriptor budget.

Two corrections came out of that probe: `text` matches the description as well as the title,
and an empty result now names the filters applied and says the query succeeded, as
`assessments_list` already did.
...
`isArchived` and `source`, and `createdAt` becomes optional because it now reflects whether
