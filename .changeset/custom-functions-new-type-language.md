...
Workflow step. The interactive flow now asks for that type first. General continues immediately
because it has only one template; DSR opens a second prompt for a data point resolver, preflight
check, or both. The flag brief, command description, and README use the same language.
