 ...
 */
export function getWorkerLogPaths(child: ChildProcess): WorkerLogPaths | undefined {
...
export function safeSend(w: ChildProcess, msg: unknown): boolean {
  ...
  try {
    ...
    if (err?.code === 'ERR_IPC_CHANNEL_CLOSED' || err?.code === 'EPIPE' || err?.errno === -32) {
      return false;
    }
    throw err;
  }
...
export function spawnWorkerProcess(opts: SpawnWorkerOptions): ChildProcess {
  ...
  [structuredPath, outPath, errPath, infoPath, warnPath, errorPath].forEach(ensureLogFile);
