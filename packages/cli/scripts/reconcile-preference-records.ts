...
import { createSombraGotInstance } from '../src/lib/graphql/index.js';
import { getPreferencesForIdentifiers } from '../src/lib/preference-management/index.js';
 ...
 * @param records
 */
function buildLookupMapAll(records: PreferenceRecord[]): Map<string, PreferenceRecord[]> {
...
async function main(): Promise<void> {
  ...
  if (!inputCsv || !opts.partition || !opts.transcendUrl || !opts.transcendApiKey) {
    throw new Error(
      'Missing one or more required environment variables: INPUT_CSV, PARTITION, TRANSCEND_URL, TRANSCEND_API_KEY.',
    );
  }
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
    ...
    const emails: Identifier[] = uniqBy(
      batch
        .map((r) => {
          ...
          return v && v.includes('@') ? ({ name: 'email', value: v } as Identifier) : null;
        })
        .filter(Boolean) as Identifier[],
      (x) => `${x.name}:${x.value}`,
    );
    ...
    const [recordsByTranscendRaw, recordsByPersonRaw, recordsByEmailRaw] = await Promise.all([
    ...
    const results = await map(
      ...
      async (r, idxInBatch) => {
        ...
        const lookupBy: 'transcendID' | 'personID' = transcendID ? 'transcendID' : 'personID';
        ...
        const recsRaw: PreferenceRecord[] = [
          ...(transcendID ? (lookupMapRaw.get(`transcendID:${transcendID}`) ?? []) : []),
          // ...(personID ? lookupMapRaw.get(`personID:${personID}`) ?? [] : []),
          ...(email && email.includes('@') ? (lookupMapRaw.get(`email:${email}`) ?? []) : []),
        ];
        ...
        if (runEnabled) {
          ...
          if (metrics.runError) {
            logger.warn(colors.yellow(`Row ${rowIndex} update error: ${metrics.runError}`));
          }
        }
    ...
    for (const { personID, transcendID, emailWithheld, metrics } of results as any) {
      writer.write(
        `${[
          csvEscape(personID),
