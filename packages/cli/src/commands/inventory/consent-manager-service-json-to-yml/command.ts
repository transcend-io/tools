...
import { PushCommandFlags } from '../push/impl.js';
import { ConsentManagerServiceJsonToYmlCommandFlags } from './impl.js';

export const consentManagerServiceJsonToYmlCommand = buildCommand({
  loader: async () => {
    const { consentManagerServiceJsonToYml } = await import('./impl.js');
  ...
  parameters: {
    flags: {
      file: {
        kind: 'parsed',
        parse: String,
        brief: 'Path to the services.json file, output of await airgap.getMetadata()',
