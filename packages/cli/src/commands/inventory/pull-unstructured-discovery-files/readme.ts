...
import type { PullUnstructuredDiscoveryFilesCommandFlags } from './impl.js';
...
const examples = buildExamples<PullUnstructuredDiscoveryFilesCommandFlags>(
  ...
  [
    ...
    {
      description: 'Specify the backend URL, needed for US hosted backend infrastructure',
      flags: {
        auth: '$TRANSCEND_API_KEY',
        transcendUrl: 'https://api.us.transcend.io',
      },
    ...
    {
      description: 'Filter by classification status (exclude unconfirmed recommendations)',
      flags: {
        auth: '$TRANSCEND_API_KEY',
        status: [
          UnstructuredSubDataPointRecommendationStatus.Validated,
    ...
    {
      description: 'Filter by classification status (include rejected recommendations)',
      flags: {
        auth: '$TRANSCEND_API_KEY',
