...
export const consentManagersToBusinessEntitiesCommand = buildCommand({
  loader: async () => {
    const { consentManagersToBusinessEntities } = await import('./impl.js');
  ...
  parameters: {
    flags: {
      consentManagerYmlFolder: {
        kind: 'parsed',
        parse: String,
        brief: 'Path to the folder of Consent Manager transcend.yml files to combine',
      },
      output: {
        kind: 'parsed',
        parse: String,
        brief: 'Path to the output transcend.yml with business entity configuration',
