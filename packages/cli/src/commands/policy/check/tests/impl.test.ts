...
import { check, type CheckCommandFlags } from '../impl.js';
...
/**
 * Build complete policy check flags.
 *
 * @param overrides - Flag overrides
 * @returns Complete flags
 */
function buildFlags(overrides: Partial<CheckCommandFlags> = {}): CheckCommandFlags {
  return {
    fix: false,
    noInteractive: true,
    json: true,
...
function createPolicyProject(): string {
  const directory = mkdtempSync(join(tmpdir(), 'policy-check-command-'));
...
describe('policy check', () => {
  it('runs the complete successful gate against absolute project paths', async () => {
    ...
    await check.call(context, buildFlags(), directory, runner);
  ...
  it('resolves a custom relative directory from the invocation directory', async () => {
    ...
    await check.call(context, buildFlags(), join('.', basename(directory)), runner);
  ...
  it('repairs OPA formatting with explicit --fix', async () => {
    ...
    await check.call(context, buildFlags({ fix: true }), directory, runner);
  ...
  it('preserves interactive formatting with a diff and confirmation', async () => {
    ...
    await check.call(context, buildFlags({ json: false, noInteractive: false }), directory, runner);
  ...
  it('checks every child with a .manifest under a workspace directory', async () => {
    const workspace = mkdtempSync(join(tmpdir(), 'policy-check-workspace-'));
    ...
    await check.call(context, buildFlags(), workspace, runner);
