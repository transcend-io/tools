...
export const restartCommand = buildCommand({
  loader: async () => {
    const { restart } = await import('./impl.js');
    return restart;
  },
  parameters: {
    flags: {
      auth: createAuthParameter({
        scopes: [ScopeName.MakeDataSubjectRequest, ScopeName.ViewRequestCompilation],
      }),
      actions: {
        kind: 'enum',
      ...
      requestReceiptFolder: {
        kind: 'parsed',
        parse: String,
        brief: 'The path to the folder where receipts of each upload are stored',
  ...
  docs: {
    brief: 'Bulk update a set of privacy requests based on a set of request filters',
