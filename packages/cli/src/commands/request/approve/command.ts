...
import { RequestAction, RequestOrigin, ScopeName } from '@transcend-io/privacy-types';
...
export const approveCommand = buildCommand({
  loader: async () => {
    const { approve } = await import('./impl.js');
    return approve;
  },
  parameters: {
    flags: {
      ...
      silentModeBefore: {
        kind: 'parsed',
        parse: dateParser,
        brief: 'Any requests made before this date should be marked as silent mode',
        optional: true,
      },
      createdAtBefore: {
        kind: 'parsed',
