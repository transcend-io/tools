...
import type { PullIdentifiersCommandFlags } from '../pull-identifiers/impl.js';
import type { PushIdentifiersCommandFlags } from './impl.js';

export const pushIdentifiersCommand = buildCommand({
  loader: async () => {
    const { pushIdentifiers } = await import('./impl.js');
    return pushIdentifiers;
  },
  parameters: {
    flags: {
      auth: createAuthParameter({
        scopes: [ScopeName.ManageRequestIdentities, ScopeName.ManageRequestCompilation],
      }),
      enricherId: {
        kind: 'parsed',
        parse: uuidParser,
