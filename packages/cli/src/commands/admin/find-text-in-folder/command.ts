...
export const findTextInFolderCommand = buildCommand({
  loader: async () => {
    const { findTextInFolder } = await import('./impl.js');
  ...
  parameters: {
    flags: {
      ...
      exts: {
        kind: 'parsed',
        parse: String,
        brief: 'Comma-separated file extensions to search (without leading dots)',
      ...
      maxBytes: {
        ...
        brief: 'Stop scanning each file after this many bytes (useful for huge files)',
        optional: true,
      },
    },
  },
  docs: {
    brief: 'Find a needle in a haystack — search many large files in a folder for a text match',
