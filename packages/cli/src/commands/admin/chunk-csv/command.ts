...
export const chunkCsvCommand = buildCommand({
  loader: async () => {
    const { chunkCsv } = await import('./impl.js');
    return chunkCsv;
  },
  parameters: {
    flags: {
      ...
      outputDir: {
        kind: 'parsed',
        parse: String,
        brief: "Directory to write chunk files (defaults to each input file's directory)",
        optional: true,
      },
      clearOutputDir: {
        kind: 'boolean',
      ...
      chunkSizeMB: {
        ...
        brief: 'Approximate chunk size in megabytes. Keep well under JS string size limits',
        default: '10',
      },
      concurrency: {
        kind: 'parsed',
        parse: (v: string) => Math.max(1, Number(v) || 0),
        brief: 'Max number of worker processes (defaults based on CPU and file count)',
        optional: true,
      },
      viewerMode: {
        kind: 'boolean',
        brief: 'Run in non-interactive viewer mode (no attach UI, auto-artifacts)',
        default: false,
      },
    },
  },
