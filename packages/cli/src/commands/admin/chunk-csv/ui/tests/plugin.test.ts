...
const { chunkCsvPlugin } = await import('../plugin.js');
...
describe('chunkCsvPlugin delegator', () => {
  ...
  it('exposes only header/workers (no extras renderer)', () => {
    ...
    expect(Object.prototype.hasOwnProperty.call(chunkCsvPlugin, 'renderExtras')).toBe(false);
    // And accessing it is undefined
    expect((chunkCsvPlugin as unknown as Record<string, unknown>).renderExtras).toBeUndefined();
  });
});
