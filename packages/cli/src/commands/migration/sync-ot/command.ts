...
import { OneTrustPullResource, OneTrustPullSource } from '../../../enums.js';
import {
  createAuthParameter,
...
export const syncOtCommand = buildCommand({
  loader: async () => {
    const { syncOt } = await import('./impl.js');
    return syncOt;
  },
  parameters: {
    flags: {
      hostname: {
        kind: 'parsed',
        parse: String,
        brief: 'The domain of the OneTrust environment from which to pull the resource',
        optional: true,
      },
      oneTrustAuth: {
        kind: 'parsed',
      ...
      file: {
        kind: 'parsed',
        parse: String,
        brief: 'Path to the file to pull the resource into. Must be a json file!',
        optional: true,
      },
      resource: {
        kind: 'enum',
        values: Object.values(OneTrustPullResource) as OneTrustPullResource[],
        brief: 'The resource to pull from OneTrust. For now, only assessments is supported',
        default: OneTrustPullResource.Assessments,
      },
      dryRun: {
        kind: 'boolean',
        brief: 'Whether to export the resource to a file rather than sync to Transcend',
        default: false,
      },
      debug: {
        kind: 'boolean',
