...
import { parseDurationToMs } from '../../../lib/cli/parsers.js';

export const generateAccessTokensCommand = buildCommand({
  loader: async () => {
    const { generateAccessTokens } = await import('./impl.js');
  ...
  parameters: {
    flags: {
      ...
      file: {
        kind: 'parsed',
        parse: String,
        brief: 'Path to the CSV file containing user identifiers to generate access tokens for',
      },
      subjectType: {
        kind: 'parsed',
        parse: String,
      ...
      emailColumnName: {
        kind: 'parsed',
        parse: String,
        brief: 'Name of the column in the CSV that contains user email addresses',
        default: 'email',
      },
      coreIdentifierColumnName: {
        ...
        parse: String,
        optional: true,
        brief: 'Name of the column in the CSV that contains user core identifiers',
      },
      duration: {
        kind: 'parsed',
