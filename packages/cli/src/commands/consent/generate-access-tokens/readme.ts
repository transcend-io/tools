...
import type { GenerateAccessTokenCommandFlags } from './impl.js';

const examples = buildExamples<GenerateAccessTokenCommandFlags>(
  ['consent', 'generate-access-tokens'],
  [
    {
      description: 'Generate access tokens for users as the "customer" subject type',
      flags: {
    ...
    {
      description: 'CSV with custom column names for email/coreIdentifier + a 90-day duration',
      flags: {
        auth: '$TRANSCEND_API_KEY',
        file: './my-users.csv',
    ...
    {
      description: 'Specifying the backend URL (US-hosted backend infrastructure)',
      flags: {
        auth: '$TRANSCEND_API_KEY',
        file: './users.csv',
        subjectType: 'customer',
