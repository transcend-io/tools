...
export const deletePreferenceRecordsCommand = buildCommand({
  loader: async () => {
    const { deletePreferenceRecords } = await import('./impl.js');
  ...
  parameters: {
    flags: {
      ...
      timestamp: {
        kind: 'parsed',
        parse: dateParser,
        brief: 'The timestamp when the deletion operation is made. Used for logging purposes.',
      },
      file: {
        kind: 'parsed',
        parse: String,
        optional: true,
        brief: 'Path to the CSV file used to identify preference records to delete',
      },
      directory: {
        kind: 'parsed',
        parse: String,
