...
import type { getFilePrefix } from '../../computeFiles.js';
// Now import the SUT
import { resolveReceiptPath } from '../resolveReceiptPath.js';
...
describe('resolveReceiptPath', () => {
  ...
  it('scans directory and returns the most recent matching suffix when exact does not exist', () => {
    ...
    expect(H.statSync).toHaveBeenCalledWith(join(folder, 'FILE-receipts__1.json'));
    expect(H.statSync).toHaveBeenCalledWith(join(folder, 'FILE-receipts__2.json'));
    expect(H.statSync).toHaveBeenCalledWith(join(folder, 'FILE-receipts__3.json'));
  });

  it('ignores stat errors but still picks the newest among remaining candidates', () => {
    H.existsSync.mockReturnValueOnce(false);

    H.readdirSync.mockReturnValueOnce(['FILE-receipts__old.json', 'FILE-receipts__new.json']);
