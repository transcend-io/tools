...
export class ExportManager {
   ...
   */
  artifactPath(kind: ExportKindWithCsv, exportStatus?: ExportStatusMap): string {
   ...
   */
  async open(kind: ExportKindWithCsv, status?: ExportStatusMap): Promise<ExportArtifactResult> {
   ...
   * @returns An object containing the success status and the path of the artifact
   */
  async reveal(kind: ExportKindWithCsv, status?: ExportStatusMap): Promise<ExportArtifactResult> {
    const path = this.artifactPath(kind, status);
   ...
   * @returns An object containing the success status and the path of the artifact
   */
  async copy(kind: ExportKindWithCsv, status?: ExportStatusMap): Promise<ExportArtifactResult> {
    const path = this.artifactPath(kind, status);
  ...
  exportCombinedLogs(
    ...
    for (const [, paths] of slotLogPaths) {
      if (!paths) continue;
      if (kind === 'all') {
        [paths.outPath, paths.errPath, paths.structuredPath].filter(Boolean).forEach((p) => {
          const t = readSafe(p);
      ...
      if (kind === 'warn') {
        ...
        if (!text) {
          ...
          if (stderr) {
            const blocks = extractBlocks(stderr, (cl) => isLogWarn(cl) && !isLogError(cl));
