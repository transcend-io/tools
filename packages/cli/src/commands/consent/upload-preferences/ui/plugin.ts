 ...
 */
const fmtTime = (ts?: number): string => (ts ? new Date(ts).toLocaleTimeString() : '—');
...
function totalsBlock(t?: AnyTotals): string {
  ...
  if (isUploadModeTotals(t)) {
    const errorsList = Object.entries(t.errors || {}).map(
      ([msg, count]) => `  ${colors.red(`Count[${fmtNum(count)}]`)} ${colors.red(msg)}`,
    );
    return [
      errorsList.length ? `${colors.bold('Error breakdown:')}\n${errorsList.join('\n')}` : '',
...
function estimateTotalJobs(
  ...
  const remainingFiles = Math.max(0, ctx.filesTotal - completedFiles - inProgress.length);
 ...
 */
function jobsPerHour1h(ctx: CommonCtx<AnyTotals, UploadPreferencesSlotProgress>): number {
 ...
 */
function metricsLine(ctx: CommonCtx<AnyTotals, UploadPreferencesSlotProgress>): string {
  const { totals } = ctx;
 ...
 */
function renderHeader(ctx: CommonCtx<AnyTotals, UploadPreferencesSlotProgress>): string[] {
 ...
 */
function renderExtras(ctx: CommonCtx<AnyTotals, UploadPreferencesSlotProgress>): string[] {
  const status = (ctx.exportStatus || {}) as ExportStatusMap;

  const line = (key: string, label: string, s?: ExportArtifactStatus): string => {
    const exported = !!s?.exported;
    const raw = s?.path || '(not saved yet)';

    const abs = raw.startsWith('(') || raw.startsWith('file:') ? raw : resolve(raw);

    const openText = exported ? colors.green('open') : colors.dim('open');

    // Build a file:// href (or keep sentinel/already-a-URL)
    const href = abs.startsWith('(') ? '' : abs.startsWith('file:') ? abs : pathToFileURL(abs).href;
 ...
 */
export const uploadPreferencesPlugin: DashboardPlugin<AnyTotals, UploadPreferencesSlotProgress> = {
  renderHeader,
  renderWorkers,
  renderExtras,
};
