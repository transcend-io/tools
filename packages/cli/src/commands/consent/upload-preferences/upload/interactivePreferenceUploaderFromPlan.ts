...
export async function interactivePreferenceUploaderFromPlan(
  ...
  const pendingUpdates: Record<string, PreferenceUpdateItem> = buildPendingUpdates({
  ...
  if (dryRun) {
    logger.info(
      colors.green(
        `Dry run complete — ${Object.values(pendingUpdates).length} pending updates. ` +
  ...
  const allEntries = Object.entries(pendingUpdates) as Array<[string, PreferenceUpdateItem]>;
  ...
  if (filtered.length < allEntries.length) {
    logger.warn(
      colors.yellow(
        `Filtered ${allEntries.length - filtered.length} already-successful updates. ` +
   ...
   */
  const markSuccessFor = async (entries: Array<[string, PreferenceUpdateItem]>): Promise<void> => {
    ...
    if (shouldLog) {
      ...
      await receipts.setPending(limitRecords(pendingUpdates, maxRecordsToReceipt));
      await receipts.setPendingSafe(limitRecords(pendingSafeUpdates, maxRecordsToReceipt));
  ...
  const markFailureForSingle = async (
    ...
    logger.error(
      colors.red(`Failed to upload preferences for ${userId} (partition=${partition}): ${msg}`),
    );
  ...
  const {
    ...
    !update.identifiers
      ? 'valid'
      : update.identifiers.some((id) => id.name === 'email' && !id.value.includes('@'))
        ? 'invalidAt'
        : update.identifiers.some((id) => id.name === 'email' && id.value.includes('/'))
          ? 'invalidSlash'
          : 'valid',
  ...
  if (invalidAt.length > 0) {
    await markFailureForBatch(invalidAt, new Error('Invalid email format - missing @'));
  }
    ...
    async (currentChunk) => {
      await uploadChunkWithSplit(
        ...
        },
        {
          ...
          onFailureSingle: ([userId, update], err) => markFailureForSingle(userId, update, err),
  ...
  logger.info(
    colors.green(
      `Successfully uploaded ${Object.keys(receipts.getSuccessful()).length} user preferences ` +
