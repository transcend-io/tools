...
describe('uploadChunkWithSplit', () => {
  ...
  it('retryable error → retried in-place → success (no split)', async () => {
    ...
    const putBatch = vi.fn().mockRejectedValueOnce(retryableErr).mockResolvedValueOnce(undefined);
  ...
  it('treats 400 "slow down" as soft-rate-limit → retries in-place', async () => {
    ...
    const putBatch = vi.fn().mockRejectedValueOnce(softRateErr).mockResolvedValueOnce(undefined);
  ...
  it('treats 400 "Throughput exceeds" as soft-rate-limit → retries in-place', async () => {
    ...
    const putBatch = vi.fn().mockRejectedValueOnce(throughputErr).mockResolvedValueOnce(undefined);
