 ...
 */
export function transformCsv(preferences: Record<string, string>[]): Record<string, string>[] {
  ...
  if (isUdp) {
    logger.info(
      colors.yellow('Detected UDP format. Transforming preferences to include Transcend ID.'),
    );

    return preferences.map((pref) => {
      const email = (pref.email_address || '').toLowerCase().trim();
      const emailAddress = !email || disallowedEmails.includes(email) ? '' : pref.email_address;
      const birthDate = new Date(pref.birth_dt);
      if (!!pref.birth_dt || Number.isNaN(birthDate.getTime())) {
        logger.warn(colors.yellow(`No birth date for record: ${pref.email_address}`));
      }
      return {
        ...pref,
        Minor:
  ...
  const isAdobe =
    keys.includes('hashedCostcoID') && keys.includes('address') && keys.includes('lastUpdatedDate');
  if (isAdobe) {
    ...
    return preferences.map((pref) => {
      if (!pref.lastUpdatedDate) {
        logger.warn(
          colors.yellow(
            `Record missing lastUpdatedDate - setting to now() - ${JSON.stringify(pref)}`,
          ),
        );
      }
      return {
