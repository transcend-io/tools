...
import { applyReceiptSummary } from './artifacts/receipts/index.js';
import { buildCommonOpts } from './buildTaskOptions.js';
import {
  AnyTotals,
  isUploadModeTotals,
  isCheckModeTotals,
...
export async function uploadPreferences(
  ...
  if (chunkSizeMB > 0) {
    ...
    if (oversized.length > 0) {
      logger.info(
        colors.yellow(`Auto-chunking ${oversized.length} file(s) exceeding ${chunkSizeMB}MB...`),
      );
  ...
  if (!configExists || regenerate) {
    ...
    if (regenerate) {
      ...
      const { configurePreferenceUpload } = await import('../configure-preference-upload/impl.js');
  ...
  await runPool<UploadPreferencesTask, UploadPreferencesProgress, UploadPreferencesResult, Totals>({
