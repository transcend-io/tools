...
import type { TaskCommonOpts } from './buildTaskOptions.js';
import { makeSchemaState } from './schemaState.js';
import {
...
export async function runChild(): Promise<void> {
  ...
  const logFile = process.env.WORKER_LOG || join(process.cwd(), `logs/worker-${workerId}.log`);
    ...
    async (
      ...
      if (msg.type === 'task') {
        ...
        try {
          ...
          const columnsToIgnore = schema.state.getValue('columnsToIgnore') ?? [];
          ...
          logger.error(`[w${workerId}] ERROR ${filePath}: ${err?.message || err}\n\n${e}`);
          log(`FAIL ${filePath}\n${e}`);
          process.send?.({
            type: 'result',
