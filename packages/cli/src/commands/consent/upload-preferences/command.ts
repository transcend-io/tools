...
export const uploadPreferencesCommand = buildCommand({
  loader: async () => {
    const { uploadPreferences } = await import('./impl.js');
  ...
  parameters: {
    flags: {
      ...
      skipWorkflowTriggers: {
        kind: 'boolean',
        brief: 'Whether to skip workflow triggers when uploading to preference store',
        default: false,
      },
      forceTriggerWorkflows: {
        kind: 'boolean',
        brief: 'Whether to force trigger workflows for existing consent records',
        default: false,
      },
      skipConflictUpdates: {
        kind: 'boolean',
