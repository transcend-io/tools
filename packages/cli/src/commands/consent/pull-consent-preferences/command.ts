...
import { dateParser } from '../../../lib/cli/parsers.js';

export const pullConsentPreferencesCommand = buildCommand({
  loader: async () => {
    const { pullConsentPreferences } = await import('./impl.js');
  ...
  parameters: {
    flags: {
      ...
      timestampBefore: {
        kind: 'parsed',
        parse: dateParser,
        brief: 'Filter: preferences collected before this time (timestampBefore)',
        optional: true,
      },
      timestampAfter: {
        kind: 'parsed',
      ...
      updatedBefore: {
        kind: 'parsed',
        parse: dateParser,
        brief: 'Filter: preferences updated before this time (system.updatedAt)',
        optional: true,
      },
      updatedAfter: {
        kind: 'parsed',
      ...
      maxChunks: {
        kind: 'parsed',
        parse: numberParser,
        brief: 'Maximum number of chunks to download (higher = more data, but more load).',
        default: '20000',
      },
      maxLookbackDays: {
        kind: 'parsed',
        parse: numberParser,
        brief: 'Maximum lookback period in days for fetching consent preferences.',
        default: '3650',
      },
    },
  },
