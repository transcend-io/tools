...
describe('pullConsentPreferences', () => {
  ...
  it('parses identifiers, builds filter, passes limit=concurrency, streams pages, initializes header once, and appends rows', async () => {
    ...
    const page1 = [{ partition: 'part-xyz', timestamp: '2025-06-01T01:00:00.000Z' }];
    const page2 = [{ partition: 'part-xyz', timestamp: '2025-06-01T02:00:00.000Z' }];
    ...
    expect(H.transformPreferenceRecordToCsv).toHaveBeenCalledTimes(allItems.length);
    expect(H.transformPreferenceRecordToCsv.mock.calls[0][0]).toEqual(allItems[0]);
    expect(H.transformPreferenceRecordToCsv.mock.calls[1][0]).toEqual(allItems[1]);
    ...
    expect(headers).toEqual(expect.arrayContaining(['email', 'phone', 'loyaltyId']));
