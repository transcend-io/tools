...
import { parsePreferenceAndPurposeValuesFromCsv } from '../../../lib/preference-management/parsePreferenceAndPurposeValuesInteractive.js';
import { parsePreferenceFileFormatFromCsv } from '../../../lib/preference-management/parsePreferenceFileFormatFromCsv.js';
import { parsePreferenceIdentifiersFromCsv } from '../../../lib/preference-management/parsePreferenceIdentifiersInteractive.js';
...
async function scanOneFile(file: string): Promise<{
  ...
  await new Promise<void>((resolve, reject) => {
    const parser = createReadStream(file).pipe(csvParse({ columns: true, skip_empty_lines: true }));
  ...
  const run = async (): Promise<void> => {
    while (queue.length > 0) {
      ...
      if (completed % 25 === 0 || completed === files.length) {
        logger.info(colors.green(`  Scanned ${completed}/${files.length} files...`));
      }
    }
  };

  const workers = Array.from({ length: Math.min(SCAN_CONCURRENCY, files.length) }, () => run());
...
function buildSyntheticRows(
  ...
  const maxRows = Math.max(1, ...enumColumns.map((h) => uniqueValuesByColumn[h]?.size ?? 0));
  const rows: Record<string, string>[] = [];
  for (let i = 0; i < maxRows; i += 1) {
    const row: Record<string, string> = {};
    for (const h of headers) {
      const vals = uniqueValuesByColumn[h] ? [...uniqueValuesByColumn[h]] : [''];
...
export async function configurePreferenceUpload(
  ...
  logger.info(
    colors.green(`Scanning ${files.length} CSV file(s) for headers and unique values...`),
  );

  const { headers, uniqueValuesByColumn } = await scanCsvFiles(files);
  logger.info(colors.green(`Discovered ${headers.length} columns across all files.`));
  ...
  const existingIdentifierCols = Object.keys(schemaState.getValue('columnToIdentifier'));
  let identifierColumns: string[];
  if (existingIdentifierCols.length > 0) {
    logger.info(
      colors.magenta(`Existing identifier columns: ${existingIdentifierCols.join(', ')}`),
    );
          ...
          await inquirer.prompt<{ cols: string[] }>([
            {
              ...
              choices: headers,
              validate: (v: string[]) => v.length > 0 || 'Select at least one identifier column',
            },
          ])
        ).cols;
  } else {
  ...
  // Step 2: map identifier columns to org identifier names
  logger.info(
  ...
  const identifierCols = Object.keys(schemaState.getValue('columnToIdentifier'));
  ...
  await parsePreferenceFileFormatFromCsv(
    [
      Object.fromEntries(
        timestampChoices.map((h) => [h, [...(uniqueValuesByColumn[h] ?? [])][0] ?? '']),
      ),
    ],
    schemaState,
  );

  // Step 4: select purpose/preference columns
  logger.info(colors.green('\n[Step 4/6] Purpose/preference column selection...'));
  const timestampCol = schemaState.getValue('timestampColumn');
  const mappedSoFar = [...identifierCols, ...(timestampCol ? [timestampCol] : [])];
  ...
  const { purposeColumns } = await inquirer.prompt<{
  ...
  }>([
    {
      ...
      validate: (v: string[]) => v.length > 0 || 'Select at least one purpose column',
    },
  ]);

  const nonPurposeColumns = remainingColumns.filter((h) => !purposeColumns.includes(h));

  // Step 5: map purpose values
  logger.info(colors.green('\n[Step 5/6] Mapping purpose values...'));
  const syntheticRows = buildSyntheticRows(headers, uniqueValuesByColumn, purposeColumns);
  logger.info(
    colors.green(
  ...
  // Step 6: metadata column selection
  ...
  if (nonPurposeColumns.length > 0) {
    ...
    const { metadataColumns } = await inquirer.prompt<{
    ...
    }>([
      {
        ...
        type: 'checkbox',
        message: 'Select columns to INCLUDE as metadata ' + '(unselected columns will be ignored)',
        choices: nonPurposeColumns,
      },
    ]);

    const ignored = nonPurposeColumns.filter((c) => !metadataColumns.includes(c));
    ...
    logger.info(
      colors.green(
        `  Metadata: ${metadataColumns.length > 0 ? metadataColumns.join(', ') : '(none)'}`,
      ),
    );
    logger.info(colors.green(`  Ignored: ${ignored.length > 0 ? ignored.join(', ') : '(none)'}`));
  }
  ...
  const metadataCols = Object.keys(schemaState.getValue('columnToMetadata') ?? {});
