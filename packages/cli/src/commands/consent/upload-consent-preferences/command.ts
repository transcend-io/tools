...
import { createConsentUrlParameter } from '../../../lib/cli/common-parameters.js';

export const uploadConsentPreferencesCommand = buildCommand({
  loader: async () => {
    const { uploadConsentPreferences } = await import('./impl.js');
  ...
  parameters: {
    flags: {
      ...
      base64SigningKey: {
        kind: 'parsed',
        parse: String,
        brief: 'The signing key used to prove authentication of consent request',
      },
      partition: {
        kind: 'parsed',
        parse: String,
