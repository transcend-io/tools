...
const H = vi.hoisted(() => ({
  ...
  addDaysUtc: vi.fn((d: Date, n: number) => new Date(d.getTime() + n * 86400000)),
...
vi.mock('../../../logger.js', () => ({ logger: H.loggerSpies }));
...
vi.mock('../iterateConsentPages.js', () => ({
   ...
   */
  async *iterateConsentPages(_sombra: Got, partition: string, filter: any, pageSize: number) {
