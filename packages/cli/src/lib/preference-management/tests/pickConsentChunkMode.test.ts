...
describe('pickConsentChunkMode', () => {
  ...
  it('returns "timestamp" when timestampBefore is present', () => {
    expect(M({ timestampBefore: '2025-01-02T00:00:00.000Z' })).toBe('timestamp');
  });
  ...
  it('returns "updated" when no timestamp bounds are present but system.updated* is present', () => {
    expect(M({ system: { updatedAfter: '2025-01-01T00:00:00.000Z' } })).toBe('updated');

    expect(M({ system: { updatedBefore: '2025-01-03T00:00:00.000Z' } })).toBe('updated');
