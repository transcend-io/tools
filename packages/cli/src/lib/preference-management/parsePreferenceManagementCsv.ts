...
} from './parsePreferenceIdentifiersFromCsv.js';
...
export async function parsePreferenceManagementCsvWithCache(
  ...
  const currentColumnToIdentifierMap = schemaState.getValue('columnToIdentifier');
  const currentColumnToPurposeName = schemaState.getValue('columnToPurposeName');
  ...
  const consentRecordByUniqueIdentifiers = uniqueIdentifiers.reduce(
    (acc, identifier) => {
      const recordsWithIdentifier = existingConsentRecords.filter((record) =>
        (record.identifiers || []).some((id) => id.name === identifier && id.value),
      );
      acc[identifier] = keyBy(
        recordsWithIdentifier,
        (record) => (record.identifiers || []).find((id) => id.name === identifier)?.value || '',
      );
      return acc;
    },
  ...
  logger.log(
    colors.green(
      ...
      } identifiers and ${Object.keys(currentColumnToPurposeName).length} purposes.`,
    ),
  );
  preferences.forEach((pref, ind) => {
    ...
    const currentConsentRecord =
      consentRecordByUniqueIdentifiers[primaryKeyMetadata.name][primaryKeyMetadata.value];
    ...
    if (seenAlready[primaryKey]) {
      if (!Object.entries(pref).every(([key, value]) => seenAlready[primaryKey][key] === value)) {
        ...
        const diffs = Object.entries(pref)
          ...
          .join(', ');
        logger.warn(
          colors.yellow(`Duplicate primary key "${primaryKey}" at index ${ind}. Diff: ${diffs}`),
        );
