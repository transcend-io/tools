...
describe('policy runtime versions', () => {
  it('parses OPA and Regal version output', () => {
    expect(parsePolicyToolVersion('Version: 1.18.2\nGo Version: go1.25.6\n')).toEqual({
      version: '1.18.2',
      major: 1,
    });
  ...
  it('requires OPA 1.x with official upgrade guidance', () => {
    expect(unsupportedOpaVersionMessage('Version: 1.18.2\n')).toBeUndefined();
  ...
  it('supports stricter capabilities compatibility through the shared parser', () => {
    expect(
      unsupportedRegalVersionMessage('Version: 0.41.0\n', '0.42.0', 'OPA 1.18.2 capabilities'),
    ).toMatch(/Regal 0\.42\.0 or newer[\s\S]*OPA 1\.18\.2 capabilities/u);
    expect(
      unsupportedRegalVersionMessage('Version: 0.42.0\n', '0.42.0', 'OPA 1.18.2 capabilities'),
    ).toBeUndefined();
  });
