...
vi.mock('../workerIds.js', () => ({
  ...
  cycleWorkers: vi.fn((_ids: number[], _cur: number | null, delta: number) => (delta >= 0 ? 1 : 2)),
}));
...
describe('installInteractiveSwitcher', () => {
  ...
  it('attaches on ATTACH: clears screen/banners, replays logs, hooks live streams', async () => {
    ...
    installInteractiveSwitcher({
      workers: workers as unknown as Map<number, import('node:child_process').ChildProcess>,
