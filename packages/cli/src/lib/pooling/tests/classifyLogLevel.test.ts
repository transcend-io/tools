...
import { classifyLogLevel } from '@transcend-io/sdk';
...
describe('classifyLogLevel', () => {
  ...
  it('parses JSON logs with numeric level', () => {
    expect(classifyLogLevel(JSON.stringify({ level: 50, msg: 'oops' }))).toBe('error');
    expect(classifyLogLevel(JSON.stringify({ level: 40, msg: 'heads-up' }))).toBe('warn');
    expect(classifyLogLevel(JSON.stringify({ level: 10, msg: 'trace' }))).toBe(null);
  });
