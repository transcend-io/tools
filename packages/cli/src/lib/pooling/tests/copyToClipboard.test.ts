...
import { copyToClipboard } from '../os.js';
...
describe('copyToClipboard', () => {
  ...
  it('linux: prefers xclip; on error falls back to xsel', () => {
    ...
    expect(mSpawn).toHaveBeenNthCalledWith(1, 'xclip', ['-selection', 'clipboard']);
    // Fallback: xsel --clipboard --input
    expect(mSpawn).toHaveBeenNthCalledWith(2, 'xsel', ['--clipboard', '--input']);
