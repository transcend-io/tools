...
describe('keymap', () => {
  ...
  describe('attached mode', () => {
    ...
    it('FORWARD uses key.sequence when provided', () => {
      expect(keymap('', K({ sequence: '\u001b[A', name: undefined }), 'attached')).toEqual({
        type: 'FORWARD',
        sequence: '\u001b[A',
      });
    });
    ...
    it('FORWARD falls back to str when sequence is undefined', () => {
      expect(keymap('typed', K({ sequence: undefined, name: undefined }), 'attached')).toEqual({
        type: 'FORWARD',
        sequence: 'typed',
      });
    });
    ...
    it('returns null when neither sequence nor str are provided', () => {
      expect(keymap('', K({ sequence: undefined, name: undefined }), 'attached')).toBeNull();
    });
  });
});
