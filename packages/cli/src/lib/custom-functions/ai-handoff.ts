...
export function buildInitAiHandoff(options: {
  ...
  return `${prefix} review the Custom Function setup in \`${options.targetDirectory}\` and add equivalent CI for this repository: install Deno ${SUPPORTED_DENO_MAJOR_VERSION} and \`@transcend-io/cli@${options.cliVersion}\`, then run \`${checkCommand}\`.`;
}
