...
describe('retrySamePromise', () => {
  ...
  it('retries on retryable errors and eventually succeeds', async () => {
    ...
    expect(onBackoff).toHaveBeenNthCalledWith(2, expect.stringContaining('attempt=2/3'));
  ...
  it('uses response.status when statusCode is absent', async () => {
    ...
    expect(onBackoff).toHaveBeenCalledWith(expect.stringContaining('status=502'));
  });
  ...
  it('emits "Unknown error" in backoff note when error has no message/body/status', async () => {
    ...
    expect(onBackoff).toHaveBeenCalledWith(expect.stringContaining('Unknown error'));
  });
});
