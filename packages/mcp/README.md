...
| [`mcp-server-custom-functions`](./mcp-server-custom-functions/) | `ViewCustomFunction`, `ManageCustomFunction`                                                                                                                                                     |
...
Install **`@transcend-io/mcp`** to get every tool (86 across all domains) in a single process. This is the fastest way to get started and is ideal when your agent can handle a large tool set.
...
| [`mcp-server-custom-functions`](./mcp-server-custom-functions/) | `transcend-mcp-custom-functions` |     2 | Custom Function listing and plaintext unwrap     |
...
| `SOMBRA_URL`                    | No                     | _(lazy GraphQL resolve)_   | Sticky **customer ingress** base URL for DSR create, Custom Function unwrap, and other Sombra REST tools. When unset, resolves `organization.sombra.customerUrl` |
| `SOMBRA_CUSTOMER_KEY`           | No                     | —                          | Customer-ingress key sent as `X-Sombra-Authorization: Bearer …`. Required for Custom Function unwrap/sign; otherwise optional (not needed on multi-tenant Transcend-hosted Sombra) |
