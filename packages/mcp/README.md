...
**A view sizes itself for the width it is given, in both directions.** The host decides that width, and it ranges from a phone-width sidebar to a maximized panel on a 27-inch display — neither of which a dashboard fills gracefully by default. So a view's outermost container is `mx-auto w-full max-w-view`: past `64rem` the cards turn into letterboxes and a progress bar becomes a rule across the screen. At the other end, `Grid` from the kit stacks to a single column below `lg`, because three KPI columns in a narrow panel squeeze a card until its value clips — silently, since an overflowing number is still laid out.
