...
```tsx
// dev/mcp-ui-common/src/Icons/Icons.tsx
...
| `bg-card*`, `text-on-card*`, `border-card-line`            | Fixed light panel kit (skips host vars): `card`, `card-sunken`, `on-card`, `on-card-muted`, `on-card-subtle`, `card-line`                      |
| `text-on-fill`                                             | Fixed white label for bold fills (`bg-brand`, `bg-fill-*`); skips host `--color-text-inverse`                                                  |
