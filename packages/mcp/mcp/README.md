...
Unified Transcend MCP Server that combines all domain tools into a single server. This is the "everything in one place" option — install this package when you want access to all <!-- MCP_TOOL_TOTAL_START -->82<!-- MCP_TOOL_TOTAL_END --> Transcend tools at once.
...
**OAuth scopes:** this server requests the union of all domain scopes (see the [scopes table](../README.md#oauth-scopes) in the MCP root README). The signed-in user must hold these permissions.

Full setup, troubleshooting, and multi-server guidance: [MCP root README](../README.md#quick-start-stdio--oauth).
...
If <!-- MCP_TOOL_TOTAL_START -->82<!-- MCP_TOOL_TOTAL_END --> tools is too many for your AI agent, install individual domain packages instead — see the [MCP section of the root README](../../../README.md#mcp-servers).
