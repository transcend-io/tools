...
2. **Build and run** — `node ./dist/cli.mjs` matches the `transcend-mcp-assessments` `bin` (use `node` because `pnpm exec transcend-mcp-assessments` may not resolve this package’s own binary in a pnpm workspace):

```bash
# from the repository root
pnpm exec turbo run build --filter="@transcend-io/mcp-server-assessments..."
set -a && source ./secret.env && set +a
pnpm -F @transcend-io/mcp-server-assessments exec node ./dist/cli.mjs
```

**Alternative:** `./scripts/mcp-run.sh ./packages/mcp/mcp-server-assessments/dist/cli.mjs` (sources `secret.env` when present; run after build).
