...
- `inventory_update_data_silo` — Update a data silo (owners, teams, vendor, silo-level purposes, metadata)
- `inventory_list_data_points` — List data points (optional `dataSiloId` filter)
- `inventory_update_or_create_data_point` — Create/update a datapoint and assign field-level purposes
...
- `inventory_write_vendor` — Create or update a vendor (upsert by title or update by id)
- `inventory_list_processing_purposes` — List Processing Purposes table entries
- `inventory_write_processing_purpose` — Create or update a processing purpose (upsert by name+purpose or update by id)
- `inventory_list_business_entities` — List business entities (titles for silo updates)
- `inventory_list_data_subjects` — List data subject types (IDs for silo blocklist updates)
