...
After `build`, start the server with `node ./dist/cli.mjs` (same entry as the `transcend-mcp-workflows` `bin`; use `node` because in a pnpm workspace `pnpm exec transcend-mcp-workflows` may not resolve this package’s own binary):

```bash
# from the repository root
pnpm exec turbo run build --filter="@transcend-io/mcp-server-workflows..."
TRANSCEND_API_KEY=your-api-key pnpm -F @transcend-io/mcp-server-workflows exec node ./dist/cli.mjs
```
