...
1. Locate each publishable bundle directory (this template uses `{root}-bundle/`
   with a `.manifest` and a `{root}/` package tree inside).
...
```sh
transcend policy lint <bundle-directory> --noInteractive
```
