...
Choose package paths from the application's required query and ownership boundaries. Keep directory structure mirrored in package paths ([directory-package-mismatch](https://www.openpolicyagent.org/projects/regal/rules/idiomatic/directory-package-mismatch)), and cover every publishable package with a `.manifest` root.
